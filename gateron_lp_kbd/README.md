...
Open `gateron_lp_kbd.kicad_pro` in KiCad 9. Files are in KiCad 8 format, which KiCad 9 opens natively (it will save them forward in v9 format). All symbols and footprints are embedded **and** provided as project-local libraries (`kbd.pretty`, `kbd.kicad_sym`) so nothing external is required.
...
1. `cd` into this folder and run the export script with KiCad's bundled Python
   (it fills the zones, saves, and writes `gateron_lp_kbd_gerbers.zip`):
...
2. Upload `gateron_lp_kbd_gerbers.zip` at pcbway.com → *Quick Order PCB*.
